...
describe("TodoWidget - Stats", () => {
	it("displays progress bar when there are todos", () => {
		render(<TodoWidget />);
		const progressBar = screen.getByRole("progressbar");
		expect(progressBar).toBeTruthy();
	});

	it("shows statistics (active, completed counts)", () => {
		render(<TodoWidget />);
		// Check for stats text
		const statsText = screen.getByText(/1 active/i);
		expect(statsText).toBeTruthy();
	});

	it("can toggle statistics charts visibility", async () => {
		const user = userEvent.setup();
		render(<TodoWidget />);

		const statsButton = screen.getAllByRole("button").find((btn) =>
			btn.getAttribute("aria-label")?.includes("statistiques")
		);

		if (statsButton) {
			await user.click(statsButton);
			// Charts should be visible after toggle
